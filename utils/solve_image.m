...
% LUT.
%
% Note: currently it is always using the newton method to solve for the LUT,
% even if alpha = 1, 2/3 or 1/2. This is because the kappa coefficient was added
% without reworking all the equations. This only makes the initial computation a
% little slower.
%
% @file
...
% @date Mar 11, 2010
%
% @optimization_file @copybrief solve_image.m
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%>
% @copybrief solve_image.m
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [w] = solve_image(v, beta, alpha,kappa)
...
%   fprintf('Reusing lookup table for beta %.3g and alpha %.3g\n', beta, alpha);
  ...
  if (exist('pointOp') == 3) 
    ...
    w = reshape(w, size(v,1), size(v,2));
     ...
     tmp = compute_w(xx, beta, alpha, kappa);
...
% @retval w the best computed root per-pixel.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function w = compute_w(v, beta, alpha, kappa)
%   
% if (abs(alpha - 1) < 1e-9)
%   % assume alpha = 1.0
%   w = compute_w1(v, beta);
...
% @retval w the best computed root per-pixel.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function w = newton_w(v, beta, alpha, kappa)
...
for a=1:iterations
  fd = (kappa)*(alpha)*sign(x).*abs(x).^(alpha-1)+beta*(x-v);
  fdd = (kappa)*alpha*(alpha-1)*abs(x).^(alpha-2)+beta;
...
z = beta/2*v.^2;
f   = kappa*abs(x).^alpha + beta/2*(x-v).^2;
w = (f<z).*x;
