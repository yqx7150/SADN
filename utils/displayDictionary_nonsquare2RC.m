function I = displayDictionary_nonsquare2RC(D, numRows, numCols, sortVarFlag) 
...
% each atom. 
%%% 针对Mlad 只能处理Square的问题，我使之能处理NonSquare

numElems = size(D,2);

% numRows = floor(sqrt(numElems));
% numCols = ceil(numElems/numRows);
...
for j = 1:numRows 
    for i = 1:numCols 
        if counter>numElems continue; end
%         if (strechEachVecFlag) 
%             D(:,counter) = D(:,counter)-min(D(:,counter)); 
%             D(:,counter) = D(:,counter)./max(D(:,counter)); 
%         end 
